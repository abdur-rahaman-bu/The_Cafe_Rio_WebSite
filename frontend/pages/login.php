...
<body>
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-4">
                            ...
                            <label class="form-label">Email</label><input id="email" type="email"
                                class="form-control mb-3" placeholder="name@example.com">
                            <label class="form-label">Password</label><input id="password" type="password"
    ...
    (function auto() {
        ...
        if (u && u.user_id) {
            ...
            location.href = role === 'admin' ? '/restaurant-app/frontend/pages/admin-dashboard.php' : (
                redirectParam || '/restaurant-app/index.php');
        }
    })();
    el('#btnLogin').addEventListener('click', async () => {
        ...
        if (!email || !password) {
            showAlert('warning', 'Email ও Password দিন');
            return;
        }
        try {
            const r = await fetch('/restaurant-app/backend/public/index.php?r=auth&a=login', {
                method: 'POST',
                headers: {
            ...
            if (!r.ok) {
                showAlert('danger', d?.error || 'Login failed');
                return;
            }
            saveUser(d.user || {});
