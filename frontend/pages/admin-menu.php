...
<body>
    ...
    <section class="py-5 bg-light">
        <div class="container">
            <div class="d-flex align-items-center justify-content-between mb-3">
                ...
                <button id="btnReload" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-clockwise"></i>
                    Reload</button>
            </div>
            ...
            <div class="card-elev mb-3">
                <div class="card-body p-3 p-md-4">
                    <div class="row g-2">
                        <div class="col-sm-3"><label class="form-label">Search</label><input id="f_q"
                                class="form-control" placeholder="name/desc/category"></div>
                        <div class="col-sm-3"><label class="form-label">Category</label><input id="f_cat"
                                class="form-control" placeholder="e.g. Pizza"></div>
                        <div class="col-sm-3">
                        ...
                        <div class="col-sm-3 d-flex align-items-end gap-2">
                            <button id="btnApply" class="btn btn-outline-secondary"><i class="bi bi-funnel"></i>
                                Apply</button>
                            <button id="btnReset" class="btn btn-outline-secondary"><i class="bi bi-x-circle"></i>
                                Reset</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-elev mb-3">
                <div class="card-body p-3 p-md-4 bg-info-subtle text-info-emphasis">
                    ...
                    <div class="row g-2 align-items-end">
                        <div class="col-md-3"><label class="form-label">Name</label><input id="c_name"
                                class="form-control" placeholder="e.g. Margherita"></div>
                        <div class="col-md-2"><label class="form-label">Price</label><input id="c_price" type="number"
                                min="0" step="1" class="form-control" placeholder="৳"></div>
                        <div class="col-md-3"><label class="form-label">Category</label><input id="c_cat"
                                class="form-control" placeholder="Pizza"></div>
                        <div class="col-md-2">
                            <label class="form-label">Status</label>
                        ...
                        <div class="col-12"><label class="form-label">Description</label><input id="c_desc"
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="form-text">২–৫MB jpg/png/webp; ফাইল বাছাই করে Create চাপুন—সাথে সাথেই আপলোড হবে।
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border-0">
                        <div class="modal-header border-0">
                            <h5 class="modal-title text-danger"><i class="bi bi-trash3 me-2"></i>Delete item</h5><button
                    ...
                    <div class="modal-content border-0">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row g-3">
                                <div class="col-md-8">
                                    <div class="row g-2">
                                        <div class="col-md-8"><label class="form-label">Name</label><input id="e_name"
                                                class="form-control"></div>
                                        <div class="col-md-4"><label class="form-label">Price</label><input id="e_price"
                                                type="number" min="0" step="1" class="form-control"></div>
                                        <div class="col-md-6"><label class="form-label">Category</label><input
                                                id="e_cat" class="form-control"></div>
                                        <div class="col-md-6">
                                            <label class="form-label">Status</label>
                                        ...
                                        <div class="col-12"><label class="form-label">Description</label><input
                                                id="e_desc" class="form-control"></div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <img id="e_prev" class="img-prev mb-2" src="" alt="preview"
                                        onerror="this.src='/restaurant-app/frontend/assets/images/_placeholder.png'">
                                    <input id="e_file" type="file" accept="image/png,image/jpeg,image/webp"
                                        class="form-control">
    ...
    function alertBox(sel, type, msg) {
        const b = $(sel);
    ...
    async function loadList() {
        hideAlert('#listAlert');
        const grid = $('#grid');
        grid.innerHTML = '<tr><td colspan="9" class="text-center text-muted">Loading…</td></tr>';
        try {
            const f = readFilters();
    ...
    function renderGrid(rows) {
        const grid = $('#grid');
        if (!rows.length) {
            grid.innerHTML = '<tr><td colspan="9" class="text-center text-muted">No items</td></tr>';
            return;
        }
    ...
    function setCreateBusy(on) {
        ...
        $('#spCreate').classList.toggle('d-none', !on);
    }
    document.getElementById('btnCreate').addEventListener('click', async () => {
        ...
        try {
            ...
            if (file) {
                ...
                res = await fetch(`${APP}/backend/public/index.php?r=menu&a=upload_image`, {
                    method: 'POST',
                    body: fd
                });
    ...
    function fillEdit(r) {
        ...
        const prev = r.image ? `${APP}/backend/public/uploads/menu/${r.image}` :
            `${APP}/frontend/assets/images/_placeholder.png`;
        $('#e_prev').src = prev;
        $('#e_file').value = '';
        hideAlert('#editAlert');
    ...
    function setEditBusy(on) {
        $('#btnEditSave').disabled = !!on;
        $('#spEdit').classList.toggle('d-none', !on);
    }
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (up) {
            ...
            try {
                ...
                const res = await fetch(`${APP}/backend/public/index.php?r=menu&a=upload_image`, {
                    method: 'POST',
                    body: fd
                });
                const data = await res.json().catch(() => ({}));
    ...
    function setDelBusy(on) {
        btnDelConfirm.disabled = !!on;
        btnDelConfirm.innerHTML = on ? '<span class="spinner-border spinner-border-sm me-2"></span>Deleting...' :
            'Delete';
    }
    btnDelConfirm.addEventListener('click', async () => {
        const u = ensureAdmin();
        if (!u) return;
        const id = WILL_DELETE;
        if (!id) return;
        setDelBusy(true);
        try {
