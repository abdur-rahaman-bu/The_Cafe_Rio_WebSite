...
<body>
   ...
   <div class="hero position-relative">
    ...
    <div class="overlay position-absolute top-50 start-50 translate-middle text-center text-white">
      <h1 class="fw-bold display-5 fst-italic">ORDER NOW</h1>
    ...
    async function loadList() {
        ...
        hideAlert('#listAlert');
        const grid = $('#grid');
        grid.innerHTML = '<tr><td colspan="7" class="text-center text-muted">Loading…</td></tr>';
        try {
            ...
            if (!res.ok) {
                alertBox('#listAlert', 'danger', data?.error || 'Failed to load orders');
                return;
            }
            const items = data.items || [];
            if (!items.length) {
                grid.innerHTML = '<tr><td colspan="7" class="text-center text-muted">No orders</td></tr>';
                return;
            }
            const badge = s => (s === 'confirmed' ? 'text-bg-success' : s === 'pending' ? 'text-bg-warning' :
                'text-bg-secondary');
            const pBadge = s => (s === 'paid' ? 'text-bg-primary' : s === 'refunded' ? 'text-bg-info' :
                'text-bg-secondary');
